<script>
  ...
  async function updateWorkout() {
    ...
    try {
      ...
      const updatedData = {
        title, type, mode, isBenchmark, notes, exercises
      };
...
<style>
  ...
  .form-group-checkbox { display: flex; align-items: center; gap: 0.75rem; margin-top: 1rem; padding: 0.75rem; background-color: var(--bg); border: 1px solid var(--border-color); border-radius: 8px; }
  .form-group-checkbox input[type="checkbox"] { width: 1.25em; height: 1.25em; }
